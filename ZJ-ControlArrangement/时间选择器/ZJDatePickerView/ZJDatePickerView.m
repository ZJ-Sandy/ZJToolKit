...
- (instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        ...
        self.tintColor = ZJ_COLOR_RGB(0, 118, 255);
...
- (void)showWithBlock:(MKDatePickerViewBlock)block{
    self.block = block;
    self.frame = ZJ_SCREEN_BOUNDS;
    [self addSubview:self.shadeView];
    [self addSubview:self.sheetView];
    
    self.sheetView.frame = CGRectMake(0, ZJ_SCREEN_HEIGHT, ZJ_SCREEN_WIDTH, self.sheetViewH);
    
    UIView *btnView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, ZJ_SCREEN_WIDTH, 40)];
    btnView.backgroundColor = ZJ_COLOR_RGB(240, 240, 240);
    ...
    btnConfirm.frame = CGRectMake(ZJ_SCREEN_WIDTH-80, 0, 80, 40);
    ...
    self.datePicker.frame = CGRectMake(0, 40, ZJ_SCREEN_WIDTH, 200);
    ...
    [[ZJUITools getCurrentWindow] addSubview:self];
    [UIView animateWithDuration:0.3 delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        frame.origin.y = ZJ_SCREEN_HEIGHT - frame.size.height;
...
- (void)dismiss{
    [UIView animateWithDuration:0.3 delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        frame.origin.y = ZJ_SCREEN_HEIGHT;
        self.sheetView.frame = frame;
...
- (void)btnConfirmOnclick:(UIButton *)sender{
    ...
    ZJ_BLOCK_EXEC(self.block, self, date);
    [self dismiss];
}
...
- (UIView *)shadeView{
    if (!_shadeView) {
        ...
        [_shadeView setFrame:ZJ_SCREEN_BOUNDS];
        [_shadeView setBackgroundColor:ZJ_COLOR_RGBA(0, 0, 0, 1)];
